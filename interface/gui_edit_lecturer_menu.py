...
class EditLecturers(tk.Frame):
    def __init__(self, master, administrator_menu, administrator_user):
        ...
        self.instrument_label = tk.Label(master=self, text="Edit lecturer information:")
        self.instrument_label.grid(row=1, columnspan=2, sticky=tk.S, padx=10, pady=10)

        self.show_lecturer_list()
    ...
    def show_lecturer_list(self):
        self.lecturer_buttons = []
        for index,lecturer in enumerate(self.administrator_user.lecturers):
            show_edit_lecturer_info_with_lecturer = partial(self.show_edit_lecturer_info, lecturer)
            new_button = tk.Button(master=self, text=f"{lecturer.first_name} {lecturer.last_name}", command=show_edit_lecturer_info_with_lecturer)
            new_button.grid(row=index+2, column=0, padx=10, pady=10, sticky=tk.E)
            self.lecturer_buttons.append(new_button)

    def show_edit_lecturer_info(self, lecturer_to_edit):
        ...
        self.firstname_var = tk.StringVar(master=self, value=lecturer_to_edit.first_name)
        ...
        self.lastname_var = tk.StringVar(master=self, value=lecturer_to_edit.last_name)
        ...
        self.dob_var = tk.StringVar(master=self, value=lecturer_to_edit.date_of_birth)
        ...
        self.contact_num_var = tk.StringVar(master=self, value=lecturer_to_edit.contact_num)
        ...
        self.contact_email_var = tk.StringVar(master=self, value=lecturer_to_edit.contact_email)
        ...
        self.username_var = tk.StringVar(master=self, value=lecturer_to_edit.username)
        ...
        self.password_var = tk.StringVar(master=self, value=lecturer_to_edit.password)
        ...
        # Button to login
        self.search_button = tk.Button(master=self, text="Edit", command= lambda: self.edit_lecturer(lecturer_to_edit))
    ...
    def edit_lecturer(self, lecturer_to_edit):
        """
        Handles the registration process of a lecturer into the management system.
        The system should prompt the user for
            - the lecturer's first name,
            - the lecturer's last name,
            - the lecturer's date of birth in DD/MM/YYYY format,
            - the contact number (of the lecturer or the contact person)
        ...
        elif self.administrator_user.edit_lecturer_data(lecturer_to_edit, self.firstname_var.get(), self.lastname_var.get(), self.dob_var.get(), self.contact_num_var.get(), self.contact_email_var.get(), self.username_var.get(), self.password_var.get()):
